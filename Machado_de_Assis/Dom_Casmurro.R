...
sparse_word <- tidybooks %>%
  count(document,word,sort=TRUE) %>%
  cast_sparse(document,word,n)
sparse_word

book_joined <-data.frame(document=as.integer(rownames(sparse_word)))%>%
left_join(books %>%
            select(document,title))
book_joined

is_jane<-book_joined$title=="Pride and Prejudice"

library(glmnet)
library(doMC)
registerDoMC(cores=8)

model<- cv.glmnet(sparse_word,is_jane,family="binomial", parallel = FALSE,keep=TRUE)
